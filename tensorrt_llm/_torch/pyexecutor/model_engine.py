...
class PyTorchModelEngine(ModelEngine):
    ...
    def __init__(
        ...
        *,
        ...
        self.model = self._load_model(
            ...
            enable_two_batch_overlap=pytorch_backend_config.
        ...
        if self.use_beam_search:
            batch_size_split = (self.batch_size + 1) // 2
            ...
            self.cache_indirection_attention_overlap_0 = torch.zeros(
                (batch_size_split, self.max_beam_width, self.max_seq_len +
                 (0 if self._disable_overlap_scheduler else 1)),
                device="cuda",
                dtype=torch.int32)
            self.cache_indirection_attention_overlap_1 = torch.zeros(
                (self.batch_size - batch_size_split, self.max_beam_width,
                 self.max_seq_len +
                 (0 if self._disable_overlap_scheduler else 1)),
                device="cuda",
                dtype=torch.int32)
    ...
    def _set_up_attn_metadata(self, kv_cache_manager: KVCacheManager):
        ...
        def create_attn_metadata():
            batch_size_split = (self.batch_size + 1) // 2
            attn_metadata = self.attn_backend.Metadata(
            ...
            attn_metadata_overlap_0 = self.attn_backend.Metadata(
                max_num_requests=batch_size_split,
                max_num_tokens=self.max_num_tokens,
                max_num_sequences=batch_size_split * self.max_beam_width,
                kv_cache_manager=kv_cache_manager,
                mapping=self.mapping,
                runtime_features=self.attn_runtime_features,
                enable_flash_mla=self.model.model_config.enable_flash_mla,
            ...
            attn_metadata_overlap_1 = self.attn_backend.Metadata(
                max_num_requests=self.batch_size - batch_size_split,
                max_num_tokens=self.max_num_tokens,
                max_num_sequences=(self.batch_size - batch_size_split) *
                self.max_beam_width,
                kv_cache_manager=kv_cache_manager,
                mapping=self.mapping,
                runtime_features=self.attn_runtime_features,
            ...
            return attn_metadata, attn_metadata_overlap_0, attn_metadata_overlap_1

        if kv_cache_manager is None:
            attn_metadata, attn_metadata_overlap_0, attn_metadata_overlap_1 = create_attn_metadata(
            )
        else:
    ...
    def _maybe_get_cuda_graph(
        ...
        num_sequences_in_batch_overlap_0 = (batch_size +
                                            1) // 2 * self.max_beam_width
        ...
        if self.pytorch_backend_config.enable_two_batch_overlap:
            ...
            attn_metadata_overlap_0 = None
            attn_metadata_overlap_1 = None
        ...
        self._cuda_graphs[batch_size][draft_len] = DecodingCUDAGraphRunner(
            ...
            self.use_mrope,
            self.max_beam_width,
            attn_metadata_overlap_0=attn_metadata_overlap_0,
            attn_metadata_overlap_1=attn_metadata_overlap_1)
        return self._cuda_graphs[batch_size][draft_len]
        ...
        if self.enable_spec_decode and not self._disable_overlap_scheduler:
            ...
            if inputs['attn_metadata'].kv_cache_manager is not None:
                ...
                if 'attn_metadata_overlap_0' in inputs:
                    ...
                    num_seq_overlap_0 = inputs[
                        'attn_metadata_overlap_0'].num_seqs
                    num_ctx_requests_overlap_0 = inputs[
                        'attn_metadata_overlap_0'].num_contexts
                    num_gen_requests_overlap_0 = inputs[
                        'attn_metadata_overlap_0'].num_generations
                    num_ctx_tokens_overlap_0 = inputs[
                        'attn_metadata_overlap_0'].num_ctx_tokens
                    num_seq_overlap_1 = inputs[
                        'attn_metadata_overlap_1'].num_seqs
                    num_ctx_requests_overlap_1 = inputs[
                        'attn_metadata_overlap_1'].num_contexts
                    num_gen_requests_overlap_1 = inputs[
                        'attn_metadata_overlap_1'].num_generations
                    num_ctx_tokens_overlap_1 = inputs[
                        'attn_metadata_overlap_1'].num_ctx_tokens
                    inputs['attn_metadata_overlap_0'].kv_lens_cuda[
                        num_ctx_requests_overlap_0:num_seq_overlap_0] += (
                            self.
                            previous_kv_lens_offsets_cuda[:
                                                          num_gen_requests_overlap_0]
                        )
                    inputs['attn_metadata_overlap_1'].kv_lens_cuda[
                        num_ctx_requests_overlap_1:num_seq_overlap_1] += (
                            self.previous_kv_lens_offsets_cuda[
                                num_gen_requests_overlap_0:num_gen_requests])
        return inputs
    ...
    def forward(
        ...
        attn_metadata, attn_metadata_overlap_0, attn_metadata_overlap_1 = self._set_up_attn_metadata(
            kv_cache_manager)
        if self.enable_spec_decode:
            spec_resource_manager = resource_manager.get_resource_manager(
                ResourceManagerType.SPEC_RESOURCE_MANAGER)
        ...
        with self._maybe_pad_batch(scheduled_requests, kv_cache_manager,
            ...
            enable_two_batch_overlap = self.pytorch_backend_config.enable_two_batch_overlap and maybe_graph is not None and attn_metadata_overlap_0 is not None
                ...
                if maybe_graph.needs_capture():
                    ...
                    if enable_two_batch_overlap:
                        ...
                        scheduled_requests_overlap_0 = ScheduledRequests(
                            generation_requests=scheduled_requests.
                            generation_requests[:offset_split],
                        ...
                        inputs_overlap_0, gather_ids_overlap_0 = self._prepare_inputs(
                            scheduled_requests_overlap_0, kv_cache_manager,
                            attn_metadata_overlap_0)
                        inputs_overlap_1, gather_ids_overlap_1 = self._prepare_inputs(
                            scheduled_requests_overlap_1, kv_cache_manager,
                            attn_metadata_overlap_1)
                        inputs["input_ids_overlap_0"] = inputs_overlap_0[
                            "input_ids"]
                        inputs["input_ids_overlap_1"] = inputs_overlap_1[
                            "input_ids"]
                        inputs["position_ids_overlap_0"] = inputs_overlap_0[
                            "position_ids"]
                        inputs["position_ids_overlap_1"] = inputs_overlap_1[
                            "position_ids"]
                    ...
                    outputs = maybe_graph.run(inputs, is_capture=True)
                else:
                    with MoeLoadBalancerIterContext(moe_load_balancer):
                        outputs = maybe_graph.run(inputs, is_capture=False)
    ...
    def model_forward(self, **kwargs):
        ...
        if 'attn_metadata_overlap_0' in kwargs:
            ...
            attrs["attention_metadata_overlap_0"] = weakref.ref(
                kwargs['attn_metadata_overlap_0'])
            attrs["attention_metadata_overlap_1"] = weakref.ref(
                kwargs['attn_metadata_overlap_1'])
