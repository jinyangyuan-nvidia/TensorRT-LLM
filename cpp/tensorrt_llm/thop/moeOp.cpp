...
public:
    ...
    void runGemmProfile(torch::Tensor const& input, torch::Tensor const& expert_weights,
        ...
        bool const min_latency_mode, bool const use_fp4, bool const use_fp8_block_scaling, int64_t const gemm_idx,
        int64_t const profile_id, bool const do_preparation)
    {
        ...
        if (do_preparation)
        {
            ...
            mProfiler->init(*mKernelRunner.get(), mProfiler->mGemmToProfile,
                ...
                min_latency_mode, use_fp4, use_fp8_block_scaling, parallelism_config);
