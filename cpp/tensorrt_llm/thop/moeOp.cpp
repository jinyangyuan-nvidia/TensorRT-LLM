...
public:
    ...
    torch::Tensor runMoe(torch::Tensor const& input, torch::optional<torch::Tensor> const& token_selected_experts,
        torch::optional<torch::Tensor> const& token_final_scales, torch::optional<torch::Tensor> const& valid_tokens,
        ...
        bool const enable_alltoall, bool min_latency_mode, torch::optional<c10::ArrayRef<int64_t>> const& profile_ids)
    {
        ...
        TORCH_CHECK(token_selected_experts.has_value() != valid_tokens.has_value(),
            "either token_selected_experts or valid_tokens must be provided");

        CHECK_INPUT(input, mActivationDtype)
        if (token_selected_experts.has_value())
        {
            CHECK_INPUT(token_selected_experts.value(), at::ScalarType::Int)
        }
        else
        {
            CHECK_INPUT(valid_tokens.value(), at::ScalarType::Int)
        }
        ...
        if (token_selected_experts.has_value())
        {
            ...
            TORCH_CHECK(token_selected_experts.value().dim() == 2, "token_selected_experts must be 2D.");
        }
        else
        {
            TORCH_CHECK(input.dim() == 3, "input must be 3D.");
            TORCH_CHECK(valid_tokens.value().dim() == 1, "valid_tokens must be 1D.");
        }
        ...
        if (token_selected_experts.has_value())
        {
            TORCH_CHECK(input.sizes()[0] == token_selected_experts.value().sizes()[0],
                "input and token_selected_experts must have the same num tokens.");
        }
        else
        {
            TORCH_CHECK(input.sizes()[0] == valid_tokens.value().sizes()[0],
                "input and valid_tokens must have the same num experts.");
        }
        if (token_final_scales)
        {
            ...
            if (token_selected_experts.has_value())
            {
                TORCH_CHECK(token_selected_experts.value().sizes()[0] == token_final_scales.value().sizes()[0],
                    "token_selected_experts and token_final_scales must have the same num tokens.");
                TORCH_CHECK(token_selected_experts.value().sizes()[1] == token_final_scales.value().sizes()[1],
        ...
        int64_t num_rows, max_tokens_per_expert;
        if (token_selected_experts.has_value())
        {
            experts_per_token = token_selected_experts.value().sizes()[1];
            num_rows = input.sizes()[0];
            max_tokens_per_expert = num_rows;
        }
        else
        {
            experts_per_token = 1;
            num_rows = input.sizes()[0] * input.sizes()[1];
            max_tokens_per_expert = input.sizes()[1];
        }
        ...
        {
            output_shape = {input.sizes()[0], input.sizes()[1], hidden_size};
        }
        ...
        mKernelRunner->runMoe(input.const_data_ptr(),
            ...
            token_selected_experts.has_value()
                ? reinterpret_cast<int const*>(token_selected_experts.value().const_data_ptr())
                : nullptr,
            token_final_scales.has_value() ? reinterpret_cast<float const*>(token_final_scales.value().const_data_ptr())
                                           : nullptr,
            valid_tokens.has_value() ? reinterpret_cast<int const*>(valid_tokens.value().const_data_ptr()) : nullptr,
            max_tokens_per_expert, fc1_expert_weights.const_data_ptr(),
        ...
        bool const enable_alltoall, bool min_latency_mode, torch::optional<c10::ArrayRef<int64_t>> const& profile_ids)
    {
        ...
        mKernelRunner->runMoe(input.const_data_ptr(),
            ...
            token_final_scales.has_value() ? reinterpret_cast<float const*>(token_final_scales.value().const_data_ptr())
                                           : nullptr,
            /*valid_tokens=*/nullptr, /*max_tokens_per_expert=*/num_rows, fc1_expert_weights.const_data_ptr(),
            fc1_expert_biases.has_value() ? fc1_expert_biases.value().const_data_ptr() : nullptr, activation_params,
            fc2_expert_weights.const_data_ptr(),
            fc2_expert_biases.has_value() ? fc2_expert_biases.value().const_data_ptr() : nullptr, quant_params,
            num_rows, hidden_size, inter_size, num_experts_total, static_cast<int>(experts_per_token),
...
private:
    ...
    WorkspaceInfo getWorkspaceInfo(int64_t const num_rows, int64_t const hidden_size, int64_t const inter_size,
        ...
        kernels::MOEParallelismConfig const& parallelismConfig, bool min_latency_mode, bool is_offset_layout)
    {
        size_t moe_workspace_size = mKernelRunner->getWorkspaceSize(num_rows, hidden_size, inter_size, num_experts,
            ...
            min_latency_mode, mUseW4GroupScaling, is_offset_layout);
